...
//    }
    ...
    @PostMapping("/add")
    public Result add(@RequestBody GuidingBook guidingBook){
        if (guidingBookService.add(guidingBook.getGuidingBookName(),guidingBook.getGuidingBookUrl())){
            return new Result("success", "操作成功");
        }else {
            return new Result("error","操作失败");
        }
    ...
    public ModelAndView findAll(){
        ...
        mv.addObject("guidingBook",guidingBooks);
        mv.setViewName("guidingbook");
        return mv;
...
//        }
    ...
    public Result fileUpload(@RequestParam("file") MultipartFile file, @RequestParam String guidingBookName){
        ...
        String path = "D:\\JAVA\\rgyguidingbook" ;
